...
public class CreateColoringRecipe {
    public static Recipe<?> createMixingRecipe(ResourceLocation id, ColoringRecipe template, DyeColor color) {
        try {
            String colorName = color.getName();
            ResourceLocation outputLocation = ResourceLocation.parse(template.resultPattern.replace("{color}", colorName));

            if (!ForgeRegistries.ITEMS.containsKey(outputLocation))
                return null;

            Ingredient baseIngredient = ColoringRecipeRegistry.createIngredientExcludingColor(template.basePattern, color);
            ...
            for (ColoringRecipe.FluidIngredient fluidIngredient : template.fluidIngredients) {
            ...
            throw new RuntimeException("Failed to create mixing recipe [" + id + "]", e);
        }
    }

    public static Recipe<?> createDeployingRecipe(ResourceLocation id, ColoringRecipe template, DyeColor color) {
        try {
            String colorName = color.getName();
            ResourceLocation outputLocation = ResourceLocation.parse(template.resultPattern.replace("{color}", colorName));
            ...
            Ingredient toolIngredient = ColoringRecipeRegistry.createIngredientFromPattern(template.itemIngredients.get(0), colorName);
            ...
            throw new RuntimeException("Failed to create deploying recipe [" + id + "]", e);
        }
    }
}
